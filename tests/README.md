...
| `--emu` | `.emu/build/pmb887x-emu` | the pmb887x-emu binary to run (also `E2E_EMU`) |
...
dependencies are only listed by `scripts/setup-emu.sh`, install them
yourself).

`--emu=/path/to/pmb887x-emu` runs an emulator of your own instead of
building one — an installed package (ArchLinux: `yay -S pmb887x-emu`, OSX:
`brew install siemens-mobile-hacks/tap/pmb887x-emu`) or a build elsewhere.
That is also the way out when `tests/` is shared with another machine: a
QEMU linked against the libraries of one distribution does not start on
another, and the run stops with that message up front instead of timing out
on every emulator.
